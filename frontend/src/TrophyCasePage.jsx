...
export default function TrophyCasePage({ teams = [], dateline }) {
  ...
  return (
    <div className="max-w-6xl mx-auto font-serif text-[#2b2418]">
      <header className="border-y-4 border-double border-[#5a4a32] bg-[#f3ecdb] px-4 py-5 text-center">
        <p className="text-[10px] tracking-[0.35em] uppercase text-[#8a7a5c] mb-2">The Dynasty Madness Times · Page Four</p>
        <h1 className="text-3xl md:text-6xl font-black tracking-tight uppercase">The Trophy Case</h1>
        <p className="mt-2 text-sm md:text-base italic text-[#8a7a5c]">Five Seasons of Dynasty Madness</p>
        <div className="mt-3 border-t border-[#c9bb9c] pt-2 text-[10px] uppercase tracking-[0.25em] text-[#8a7a5c]">{today} · Records etched forever</div>
      </header>

      <section className="relative overflow-hidden border-b-2 border-[#5a4a32] bg-[#332918] px-5 py-8 text-[#f3ecdb] md:px-10">
      ...
      <section className="px-4 py-7">
        <div className="mb-4 flex items-end justify-between border-y-2 border-[#5a4a32] py-2">
          <div><h3 className="text-xl font-black uppercase tracking-[0.18em]">The Trophy Shelf</h3><p className="text-[10px] uppercase tracking-widest text-[#8a7a5c]">Select a season to inspect its podium</p></div>
          <Trophy size={22} className="text-[#b58a2a]" />
        </div>
        <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
          {LEAGUE_HISTORY.map(season => {
            const active = season.year === selectedYear
            return (
              <button key={season.year} onClick={() => setSelectedYear(season.year)} className={`group relative border-2 p-3 text-left transition-all duration-300 motion-reduce:transition-none ${active ? 'border-[#b58a2a] bg-[#f5e8b9] -translate-y-1 shadow-[0_6px_0_#b58a2a]' : 'border-[#c9bb9c] bg-[#f3ecdb] hover:-translate-y-1 hover:border-[#b58a2a]'}`}>
                <div className="flex justify-center"><TrophyMark size={62} /></div>
                <div className="mt-1 border-t border-[#b58a2a88] pt-2 text-center"><p className="text-xl font-black">{season.year}</p><p className="truncate text-xs font-bold">{season.champion}</p><p className="mt-1 text-[9px] uppercase tracking-widest text-[#8a7a5c]">Engraved champion</p></div>
              </button>
            )
          })}
        </div>
        <div className="mt-5 border-2 border-[#b58a2a] bg-[#f5e8b9] p-4">
          ...
          <div className="mt-3 grid gap-2 md:grid-cols-3">
            {[selectedSeason.champion, selectedSeason.runnerUp, selectedSeason.third].map((name, index) => (
              <div key={name} className="flex items-center gap-3 border border-[#b58a2a88] bg-[#f3ecdb] p-3">
      ...
      <section className="px-4 pb-7">
        <div className="mb-4 border-y-2 border-[#5a4a32] py-2"><h3 className="text-xl font-black uppercase tracking-[0.18em]">All-Time Honors Ledger</h3><p className="text-[10px] uppercase tracking-widest text-[#8a7a5c]">Titles first, podiums thereafter · historical aliases retained</p></div>
        <div className="overflow-x-auto border border-[#c9bb9c]">
          <table className="w-full min-w-[720px] bg-[#f3ecdb] text-sm">
            <thead><tr className="border-b-2 border-[#5a4a32] text-left text-[10px] uppercase tracking-widest"><th className="p-2">Franchise / known names</th><th className="p-2 text-center">Titles</th><th className="p-2 text-center">Seconds</th><th className="p-2 text-center">Thirds</th><th className="p-2">Best record</th><th className="p-2">Worst record</th></tr></thead>
            <tbody>{aggregates.map((entry, index) => {
              const mapped = entry.key.startsWith('franchise-') ? teamById.get(Number(entry.key.split('-')[1])) : null
              return <tr key={entry.key} className={`border-b border-[#e0d5bc] ${index === 0 ? 'bg-[#f5e8b9]' : ''}`}><td className="p-2"><p className="font-bold">{mapped?.name || entry.names[0]} {mapped && <span className="font-normal text-xs text-[#8a7a5c]">· {mapped.owner}</span>}</p><p className="text-[10px] leading-relaxed text-[#8a7a5c]">{entry.names.join(' · ')}</p></td><td className="p-2 text-center font-black">{entry.titles}</td><td className="p-2 text-center">{entry.runnerUps}</td><td className="p-2 text-center">{entry.thirds}</td><td className="p-2 tabular-nums">{recordText(entry.best.record)} <span className="text-[10px] text-[#8a7a5c]">({entry.best.season})</span></td><td className="p-2 tabular-nums">{recordText(entry.worst.record)} <span className="text-[10px] text-[#8a7a5c]">({entry.worst.season})</span></td></tr>
            })}</tbody>
          </table>
        </div>
        {aggregates[0] && <p className="mt-3 flex items-center gap-2 text-sm italic"><Star size={15} className="text-[#b58a2a]" /> Most decorated: <strong>{aggregates[0].names.join(' / ')}</strong> with {aggregates[0].titles} title{aggregates[0].titles === 1 ? '' : 's'} and {aggregates[0].podiums} podium finish{aggregates[0].podiums === 1 ? '' : 'es'}.</p>}
      </section>

      <section className="px-4 pb-10">
        <div className="mb-4 border-y-2 border-[#5a4a32] py-2"><h3 className="text-xl font-black uppercase tracking-[0.18em]">The Engraved Archives</h3><p className="text-[10px] uppercase tracking-widest text-[#8a7a5c]">Every final table, every scar</p></div>
        <div className="grid gap-5 lg:grid-cols-2">
          {LEAGUE_HISTORY.map(season => <article key={season.year} className="border-2 border-[#5a4a32] bg-[#efe6d0] p-4 shadow-[4px_4px_0_#c9bb9c] motion-reduce:shadow-none">
            <div className="flex items-start justify-between border-b border-[#c9bb9c] pb-2"><div><p className="text-3xl font-black">{season.year}</p><p className="text-[10px] uppercase tracking-widest text-[#8a7a5c]">The final standings</p></div><Trophy size={28} className="text-[#b58a2a]" /></div>
            <div className="mt-3 space-y-1">{season.standings.map(([name, record], index) => <div key={`${season.year}-${name}`} className="flex items-center gap-2 border-b border-[#c9bb9c66] py-1.5 text-sm"><span className="w-5 text-center font-black" style={{ color: tones[index + 1]?.accent || '#8a7a5c' }}>{index + 1}</span><span className={`min-w-0 flex-1 ${index < 3 ? 'font-black' : ''}`}>{name}</span><span className="tabular-nums text-xs text-[#8a7a5c]">{record}</span></div>)}</div>
          </article>)}
        </div>
      </section>

      <section className="mx-4 mb-8 grid gap-3 border-y-2 border-[#5a4a32] bg-[#f3ecdb] p-4 text-sm md:grid-cols-4">
        <div><p className="text-[9px] uppercase tracking-widest text-[#8a7a5c]">Dynasty note</p><p className="font-bold">{backToBack[0]} is the only back-to-back champion.</p></div>
        <div><p className="text-[9px] uppercase tracking-widest text-[#8a7a5c]">Lowest-record champion</p><p className="font-bold">{lowestChampion.year}: {lowestChampion.champion} ({lowestChampion.standings[0][1]})</p></div>
        <div><p className="text-[9px] uppercase tracking-widest text-[#8a7a5c]">The comeback</p><p className="font-bold">Team caz went {cazCollapse}, then won the {cazTitleYear} title.</p></div>
        <div><p className="text-[9px] uppercase tracking-widest text-[#8a7a5c]">Ironman</p><p className="font-bold">Teddy Bongwater: {teddy?.seasons.length} straight appearances, {teddy?.titles || 0} titles.</p></div>
      </section>
    </div>
  )
}
